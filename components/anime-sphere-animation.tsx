...
const AnimeSphereAnimation = () => {
  ...
  const fitElementToParent = (el: HTMLElement, padding?: number) => {
    ...
    function resize() {
      ...
      timeout = setTimeout(() => utils.set(el, { scale: ratio }), 10)
    }
  ...
  useEffect(() => {
    ...
    // Animate the intro (draw effect)
    animate(spherePathEls, {
      strokeDashoffset: (el: SVGPathElement) => [el.getTotalLength(), 0],
      duration: 2000,
      delay: stagger(60, { direction: "reverse" }),
      easing: "easeInOutCirc",
      loop: false,
    })

    // Animate the "breath" effect
    animate(spherePathEls, {
      stroke: ["#3b82f6", "rgba(80,80,80,.35)"],
      translateX: [2, -4],
      translateY: [2, -4],
      direction: "alternate",
      loop: true,
    ...
    return () => {
      cleanupFit()
      // No need to pause animations in v4, as they are GC'd when targets are removed
    }
  }, [])
